...
func TestPostgresqlCollection(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, metricMap{"deadlocks": 0, "rollbacks": 8.0, "blk_hit_rate": 0.9955268047622324}, data)

	rs.execFunk = testExec("fixtures/pg.sizeStats.output.txt")
	err = populate(rs, data, "total_size")
	assert.Nil(t, err)
	assert.Equal(t, metricMap{"deadlocks": 0, "rollbacks": 8.0, "blk_hit_rate": 0.9955268047622324, "total_size": 122880.0}, data)

	rs.execFunk = testExec("fixtures/pg.userStats.output.txt")
	err = populate(rs, data, "seq_scans")
	assert.Nil(t, err)
	assert.Equal(t, metricMap{"deadlocks": 0, "rollbacks": 8.0, "blk_hit_rate": 0.9955268047622324, "total_size": 122880.0, "seq_scans": 6.0}, data)
...
func TestRealPostgresqlConnection(t *testing.T) {
	...
	*/
	...
	assert.True(t, hash["rollbacks"] > 0, "This test will fail if you don't have postgresql installed")
}
...
func psqlSource(metrics ...string) *pgSource {
	...
	if len(metrics) == 0 {
		metrics = []string{"deadlocks", "total_size", "seq_scans"}
	}
