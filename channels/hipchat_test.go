...
func TestHipchat(t *testing.T) {
	...
	assert.True(t, strings.Contains(params["message"][0], "localhost: swap is greater than than 20%"))

	alert = swapEvent(check, inspeqtor.RuleRecovered)
	err = sn.trigger(alert, sendHere)
	assert.Nil(t, err)
	assert.True(t, strings.Contains(params["message"][0], "localhost: swap has recovered."))

	svc := inspeqtor.NewService("sidekiq")
	alert = &inspeqtor.Event{Type: inspeqtor.ProcessDoesNotExist, Eventable: svc, Rule: nil}
	err = sn.trigger(alert, sendHere)
	assert.Nil(t, err)
	assert.True(t, strings.Contains(params["message"][0], "[sidekiq] does not exist."))

	alert = &inspeqtor.Event{Type: inspeqtor.ProcessExists, Eventable: svc, Rule: nil}
	err = sn.trigger(alert, sendHere)
	assert.Nil(t, err)
	assert.True(t, strings.Contains(params["message"][0], "[sidekiq] now running with PID 0"))
}
